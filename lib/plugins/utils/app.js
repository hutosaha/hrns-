...
app.sortInterviewPayload = (payload, callback) => {

}

app.emailAdminForGenericCV = (payload, user, callback) => {

  emailAdmin.subject = (user === 'candidate') ? 'A candidate submitted their CV' : 'An agency submitted a generic candidate';
...
app.checkFileMimeType = (fileFormat, callback) => {
  var acceptedFiles = [ "application/msword",
                        "application/vnd.openxmlformats-officedocument.wordprocessingml.document",
                        "application/vnd.oasis.opendocument.text",
                        "application/pdf" ];

  acceptedFiles.indexOf(fileFormat) > -1 ?
  callback("validFileFormat") : callback("invalidFileFormat");
}
