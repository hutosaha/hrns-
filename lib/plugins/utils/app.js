...
const basicEmail        = mailgun.basicEmail;
...
app.cleanPayload = (payload) => {
  ...
  if(payload.searchDeadline){
    payload.searchDeadline = moment(payload.searchDeadline).format('llll').slice(0,-8)
  }
...
app.emailAdminForVacancyCV = (agencyName, vid) => { // don't think a callback is necessary (HUW)
  let emailObj = {
    to: 'hdrd92@hotmail.com', // TODO change to Admin Email
  ...
  getHash(vid, (res) => {
    ...
    emailObj.html = '<h1>' + agencyName + ' submitted a new candidate!</h1>' +
                    '<p>It\'s on the ' + jobTitle + ' vacancy at ' + companyName + '.</p>' +
                    '<p>Click here to view the vacancy page</p>'; // TODO change URL
      ...
      getHash(cvid, (res) => {
        if (res) {
          ...
          emailObj.html = '<h1>Good news!</h1>' +
                          ...
                          '<p>The client will now be reviewing the candidate and we\'ll let you know if they get accepted!' +
                          '<p>Come talk to us at harness@harness.com if you\'ve got any questions. See you soon!</p>'; // TODO change to admin email address
