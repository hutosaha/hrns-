...
const backend = new RegExp(/^[a-zA-Z0-9 .,?-@!:;]+$/);
...
const JoiOpts = {
    contactName: Joi.string().min(1).max(30).required().regex(backend),
    email: Joi.string().email().required(),
    contactNumber: Joi.string().alphanum().required(),
    companyName: Joi.string().min(1).max(50).required().regex(backend),
    companyDescription: Joi.string().min(1).max(50).required().regex(backend),
