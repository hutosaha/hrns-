      ...
      config: {
        ...
        handler: (request,reply) => {
          ...
          addClientSignUpDetails(payload, id, (res) => {
            if (res) {
              reply.view('message', {
                message: 'Thanks for signing-up, we will let you know by email when you can use the site!' // change this to Ben's copy
              });
            } else {
              reply.view('message', {
                message: 'Sorry somehting went wrong, please try <a href="/clientsignup">again</a>'
              });
            }
          });
        }
