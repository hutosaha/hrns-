              ...
              emailAgencyForAdminAcceptance(agencyEmail, vid, cvid, (res) => {
                  if (res) {
                      adminApproveCV(cvid, vid, rating, (res) => {
                        if (res) {
                          opts.message = 'Great! This candidate has been submitted to the client\'s shortlist, and the agency has been notified. <br> Click <a href="/admin/job/' + vid + '">here</a> to go back to the vacancy page';
                          reply.view('message', opts, layout);
                        } else {
                          reply.view('message', opts, layout);
                        }
