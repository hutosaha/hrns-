      ...
      config: {
          ...
          handler: (request, reply) => {
              ...
              emailAgencyForAdminAcceptance(agencyEmail, vid, cvid, (res) => {
                  if (res) {
                    // add the rating to the cvid, and add the cvid to the vid + 'clientShortlist' set, srem from adminShortlist
                      adminApproveCV(cvid, vid, rating, (res) => {
                        if (res) {
                          opts.message = 'Great! This candidate has been submitted to the shortlist for the client to look at.' +
                          'The agency has been notified too. lick <a href="/admin/job/' + vid + '">here</a> to go back to the vacancy page';
                          reply.view('message', opts, layout);
                        } else {
                          reply.view('message', opts, layout);
                        }
