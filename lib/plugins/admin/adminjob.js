              ...
              if (res) {
                ...
                client.hgetall(vid, (err, res) => {
                  if (res) {
                    ...
                    let newEmailAgencyForAdminRejection = Object.assign({}, emailAgencyForAdminRejection)
                    
                    newEmailAgencyForAdminRejection.html = emailAgencyForAdminRejection.html
                         .replace('-candidateName-', candidateName)
                         .replace('-jobTitle-', jobTitle)
                         .replace('-companyName-', companyName);

                    mailgun.messages().send(newEmailAgencyForAdminRejection);
            ...
            getHash(vid, (res) => {
              if (!res) { return; }
                ...
                getHash(cvid, (res) => {
                  ...
                  let newEmailAgencyOfAdminAcceptance  = Object.assign({},emailAgencyOfAdminAcceptance);
                  newEmailAgencyOfAdminAcceptance.to   = agencyEmail;
                  newEmailAgencyOfAdminAcceptance.html = newEmailAgencyOfAdminAcceptance.html
                      .replace('-candidateName-', candidateName)
                      .replace('-jobTitle-', jobTitle)
                      .replace('-companyName-', companyName);
                     
                  mailgun.messages().send(newEmailAgencyOfAdminAcceptance);
                });
            });
          ...
          handler: (request, reply) => {
            ...
            client.hget(vid, 'jobTitle', (err, res) => {
              if (res) {
                ...
                let jobTitle = res;
                let newEmailClientAboutNewCandidates = Object.assign({},emailClientAboutNewCandidates)
                newEmailClientAboutNewCandidates.to = clientEmail;
                newEmailClientAboutNewCandidates.html = newEmailClientAboutNewCandidates.html
                  .replace('-jobTitle-', jobTitle);

                mailgun.messages().send(newEmailClientAboutNewCandidates, (error) => {
                  if (error) {
