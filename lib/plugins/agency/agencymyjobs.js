  ...
  server.route({
    ...
    config: {
      ...
      handler: (request, reply) => {
        ...
        function getNoOfSchedulingCvs(stageNo, vid, stageProgress){
              client.smembers(vid+stageNo, (arr) => {
              let stageNo = arr.filter((cv) => {
                return agencyId === cv.agencyId;
              });
              stageProgress.push(stageNo.length);
            });
        }

  
              client.smembers(agencyId+'agencyScheduling', (agencyScheduling) => {

                    let vid = agencyScheduling[0];

                    getNoOfSchedulingCvs('stageOne', vid, stageProgress);
                    getNoOfSchedulingCvs('stageTwo', vid, stageProgress);
                    getNoOfSchedulingCvs('stageThree', vid,stageProgress);
                    getNoOfSchedulingCvs('stageFour', vid, stageProgress);        
                                
                    client.hgetall(vid , (res)=>{
                        opts = res;
                        opts.stages = stageProgress;
                        reply()
                    });
