...
exports.register = (server, options, next) => {
    ...
    server.route([{
        ...
        method: 'GET',
        path: '/appointment/confirmation/{appointmentId}', /// route for email response. 
        config: {
            auth: {
                strategy: 'hrns-cookie',
                scope: ['client', 'agency']
            ...
            handler: (request, reply) => {
                ...
                client.hgetall(appointmentId, (err, res) => {
                    ...
                    client.hmget(res.cvid, 'email', 'contactNumber', (err, res) => {
                        ...
                        opts.firstIntDate =  moment(opts.firstIntDate).format('dddd Do MMMM YYYY');
                        opts.firstIntTime =  moment(opts.firstInTime).format('h:mm a');
                        if(opts.secondIntDate){ 
                            opts.secondIntDate =  moment(opts.secondIntDate).format('dddd Do MMMM YYYY');
                        }
                        if(opts.secondIntTime){
                            opts.secondIntTime = moment(opts.secondIntTime).format('h:mm a');
                        }
                        if(opts.thirdIntDate){
                            opts.thirdIntDate = moment(opts.thirdIntDate).format('dddd Do MMMM YYYY');
                        }
                        if(opts.thirdIntTime){
                            opts.thirdIntTime = moment(opts.thirdIntTime).format('h:mm a');
                        }


                        console.log('First Date', opts );
