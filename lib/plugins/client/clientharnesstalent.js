        ...
        [{
                ...
                config: {
                    ...
                    handler: (request, reply) => {
                        ...
                        let jobTitles = query['jobTitle[]']
                        delete query['jobTitle[]']


                        console.log('query', query, typeof jobTitles);
                        ...
                        getSetMembersInfo(set, (harnessTalentList) => {
                            ...
                            if (Object.keys(harnessTalentList).length === 0 || Object.keys(query).length === 0) {                    
                                ...
                                let newFilterSet = filterJobTitle(jobTitles, filterSet);
                                response.array = newFilterSet;
                                reply(response);

                                function filterJobTitle(jobTitles, candidates){
                                    let newResults = [];
                                    if (typeof jobTitles === 'string'){
                                        const results = candidates.filter((candidate)=> jobTitles ===  candidate.jobTitle)                                   
                                        return   results;
                                    } else {
                                        let results = [];                         
                                        jobTitles.forEach(function(title, index, array){  
                                            results = newResults;                       
                                            newResults = results.concat( candidates.filter((candidate) =>  title === candidate.jobTitle));
