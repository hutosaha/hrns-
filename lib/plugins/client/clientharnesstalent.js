                            ...
                            if (Object.keys(harnessTalentList).length === 0 || Object.keys(query).length === 0) {
                                ...
                                switch (true) {
                                    ...
                                    case (filterKeyExcSalaryLength === 0 && query.salaryMax !== 'All' && query.salaryMin === 'All'):
                                        console.log('I\'m in only max');
                                        response.array =   filterBySalaryMax(harnessTalentList, query)
                                        return reply(response);
                                    
                                    case (filterKeyExcSalaryLength > 0 && query.salaryMax !== 'All' && query.salaryMin === 'All'):
                                        response.array = filterBySalaryMax(candidatesFilteredExcSalary, query);
                                        return reply(response);
                                    
                                    case (filterKeyExcSalaryLength === 0 && query.salaryMin !== 'All' && query.salaryMax === 'All'):
                                        response.array = filterBySalaryMin(harnessTalentList, query);                                      
                                        return reply(response);

                                    case (filterKeyExcSalaryLength > 0 && query.salaryMax === 'All' && query.salaryMin !== 'All') :
                                                                  
                                        response.array = filterBySalaryMin(candidatesFilteredExcSalary, query);
                                        return reply(response);
                                   
                                    case (filterKeyExcSalaryLength === 0 && query.salaryMin !== 'All' && query.salaryMax !== 'All') :
                                        response.array = filterBySalaryRange(harnessTalentList, query);
                                        return reply(response);
                                    
                                    case (filterKeyExcSalaryLength > 0 && query.salaryMin !== 'All' && query.salaryMax !== 'All'):
                                        response.array = filterBySalaryRange(candidatesFilteredExcSalary, query);
                                        return reply(response);
                                    
                                    case (filterKeyExcSalaryLength > 0):
                                        response.array = candidatesFilteredExcSalary;
                                        return reply(response);
                        ...
                        client.hgetAsync(agencyId, 'email')
                            .then((agencyEmail) => {
                                ...
                                client.hgetAsync(clientId, 'companyName')
                                    ...
                                    .then(() => {
                                        console.log('>>>>>>>>>>>>', interviewId , newTimes);
