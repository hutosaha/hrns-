...
exports.register = (server, options, next) => {
    ...
    server.route([{
          ...
          method: 'GET',
          path: '/client/job/{vid}/reject/{agencyEmail}/{cvid}',
          config: {
            auth: {
              strategy: 'hrns-cookie',
              scope: 'client'
            ...
            handler: (request, reply) => {
              let opts    = { title: 'Rejection successful', message: 'Sorry this candidate wasn\'t quite right, we\' try to find a better one next time!' };
              // email agency (and client?) informing of rejection
              // remove cvid from vid + clientShortlist
              reply.view('message', opts, layout);
            }
          }
        }]);
