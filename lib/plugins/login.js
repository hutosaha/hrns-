    ...
    server.route({
        ...
        config: {
            ...
            handler: (request, reply) => {
                const failOpts = { message: 'Bad request... please return <a href="/">home</a> and try again'};
                console.log('AUTH 1', request.auth);

                if (request.auth.isAuthenticated) {
                    const credentials = request.auth.credentials;

                    const userType = request.params.user;
                    const id = credentials.profile.id;

                    request.cookieAuth.set(credentials);
                    request.cookieAuth.set('type', userType);
                    request.cookieAuth.set('scope', userType);
                    console.log('Cookie 2' , request.cookieAuth);
