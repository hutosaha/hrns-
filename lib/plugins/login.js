...
exports.register = (server, options, next) => {
      ...
      server.ext({
        ...
        method: (request, reply) => {
          let endpoint, error;
          if (request.url.path.indexOf(process.env.AGENCY_AUTH_ENDPOINT) > -1) { endpoint = request.url.path.split('/')[2].split('?')[0]; }
          if (request.query.error) { error = request.query.error;}

          if (error === 'access_denied' && endpoint === process.env.AGENCY_AUTH_ENDPOINT) {
        ...
        config: {
            ...
            handler: (request, reply) => {
                ...
                if (request.auth.isAuthenticated) {
                    ...
                    //console.log('COOKIE',request.state.user);
