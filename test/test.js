...
server.init(0, (err,server) => {
	...
	test('test / for new user or expired users endpoint serves home page', (t) => {
		let actual, expected;
		server.inject( { method:'GET', url:'/'}, (response) => {
			actual = response.payload.indexOf('<h1>Home page!</h1>') >-1;
			//expected = fs.readFileSync('./public/views/home.html').toString().indexO;
			expected = true;
	...
	testEndPoint('/','GET',302, 'auth no scope user' )




	test('endpoint redirects approved users (have scope) to respective dashboard', (t) => {
		let actual, expected;
