...
server.init(0, (err, server) => {
    ...
    const testHeaderLocation = (endpoint, method, expectedHeaders, message, COOKIE, credentialsid) => {
    ...
    testPayload('/clientsignup', 'GET', 'Client Sign Up Page', 'payload response is:', nonExistingCookie);
    ...
    testEndPoint('/login/client', 'GET', 200, 'auth user  redirects : ', clientCookie);
    testEndPoint('/login/candidate', 'GET', 302, 'auth user redirects : ', candidateCookie);

    testEndPoint('/login/client', 'GET', 302, 'auth non-existing client redirects to signup', nonExistingCookie);
    testHeaderLocation('/login/client', 'GET', '/clientsignup', 'redirects to client signup form', nonExistingCookie);
    ...
    testEndPoint('/login/'+ process.env.AGENCY_LOGIN_ENDPOINT , 'GET', 302 , 'endpoint redirects:');
    //testHeaderLocation('/login/'+ process.env.AGENCY_LOGIN_ENDPOINT,'GET','https://www.linkedin.com','redirects to'); // header location is linkedin
    ...
    client.hset('testid', 'id', 'testid', () => {
        client.sadd('approvedUsers', 'testid', () => {
            testEndPoint('/login/client', 'GET', 302, 'auth user redirects with:', clientCookie, null, 'testid');
            testHeaderLocation('/login/client', 'GET', '/client', 'auth user redirects to', clientCookie, 'testid');
        });
    });
